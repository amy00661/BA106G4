...
<%
	...
	List<LsVO> list = lsSvc.getAll();
	pageContext.setAttribute("list", list);
%>
<html>
<head>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1" />
<title>貨運中心派車系統</title>
<!-- 主要套件 -->
<link rel="stylesheet"
		...
		<div class="container-fluid">
			...
			<table class="table table-striped">
				...
				<tbody>
					...
					<c:forEach var="lsVO" items="${list}" varStatus="number"
						begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
						<tr
							${(lsVO.local_schedule_id==param.local_schedule_id) ? 'style="background-color:#99ffcc"':''}>
							<td>${lsVO.local_schedule_id}</td>
							<td>${lsVO.car_type}</td>
							<td>${lsVO.car_id}</td>
							<td>${lsVO.ls_time}</td>
							<td><fmt:formatDate value="${lsVO.ls_updatetime}"
									pattern="yyyy-MM-dd HH:mm:ss" /></td>
							<td>
								<div id="updatediv">
									<FORM METHOD="post"
										ACTION="<%=request.getContextPath()%>/local_schdule/LS_Servlet.do">
										<input type="submit" value="修改" class="btn btn-info btn-block">
										<input type="hidden" name="local_schedule_id"
											value="${lsVO.local_schedule_id}"> <input
								...
								<div id="deldiv">
									<FORM METHOD="post"
										...
										<input type="submit" value="刪除"
											...
											type="hidden" class="num" name="local_schedule_id"
											value="${lsVO.local_schedule_id}"> <input
											type="hidden" name="requestURL"
											value="<%=request.getServletPath()%>"> <input
											type="hidden" name="whichPage" value="<%=whichPage%>">
							...
							<div class="modal-body">
								...
								<div>
									<label for="inputState">車種</label> <select id="inputState"
										name="car_type" class="form-control" value="value="
										<%=(lsVO == null) ? "" : lsVO.getCar_type()%>>
										<option selected>常溫車</option>
										<option selected>冷凍車</option>
										<option selected>醫藥車</option>
									</select>

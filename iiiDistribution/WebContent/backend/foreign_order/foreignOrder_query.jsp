								...
								<div class="row row-custom">
									...
									<div class="card col-xs-12 col-lg-5">
						  				<div class="card-body">
						                      ...
						                      <label>出車日期：</label>
						                      <input id="foreignOrderDate" width="130%" name="foreignOrderDate" value="${ foreignOrderDate }"/>
						                    </div>
...
<script>
	  ...
	  $(document).ready(function() {
		...
		$('#calendar').fullCalendar({
			...
			header: {
		        left: 'add_Foreign_Order today prev,next',
		        center: 'title',
...
// 		          { id: 'F_S001', title: 'F_S001 9:00' },
// 		          { id: 'F_S002', title: 'F_S002 14:00' },
// 		          { id: 'F_S003', title: 'F_S003 20:00' }
// 		        ] },
// 		      ], 
			resources: {
		          url: '<%=request.getContextPath()%>/foreign_schedule/fs.do',
		          data : {action:"get_FS_json"},
			...
			events: {
		          url: '<%=request.getContextPath()%>/foreign_order/FO_Servlet.do',
		          data : {action:"get_FOs_Bind_FS"},
		          error: function(xhr, ajaxOptions, thrownError) {
		        	  console.log(xhr.responseText);
				  }
			},
			...
			eventClick: function(calEvent, jsEvent, view) {
			    ...
			    $('#foreignOrderDate').val(date);
			    $('#car_type').trigger('change');
			    $('#orderQuery').trigger('click');
			    //console.log($("#foreign_schedule").val());
			    FO_ID = resourceId;
			    $('#calendarModal').modal('show');
			}
		});
		$('#calendarModal').on('shown.bs.modal', function (e) {//開啟modal後原先設好seled的車班會失效，所以改在modal開啟後設selected
			 if(FO_ID != ""){	//從日曆的Events點擊進入才會有值
				 $("#foreign_schedule").val(FO_ID);
				 FO_ID ="";
				 $('#foreignOrderQuery').trigger('click');
			 }
		})
		...
		$('#calendarModal').on('hide.bs.modal', function (e) {//關閉modal時將所有欄位回復至初始狀態
			$('#item_type').val('');
			$('#unShipOrders').empty();
			$('#foreignOrderDate').val(moment(today).format('YYYY-MM-DD'));
			$('#car_type').val('');
	...
	$("#car_type").change(function(){
		$.ajax({
			type : "post",
			url  : "<%=request.getContextPath()%>/foreign_schedule/fs.do",
			data : {action:"getFSbyCarType", car_type:$("#car_type").val()},
			datatype: "json",
			success : function(Jdata){
				$("#foreign_schedule").empty();
				$.each(Jdata, function(index, element){
					$("#foreign_schedule").append("<option value='"+ element.foreign_schedule_ID +"'>"+element.fs_TIME+" "+element.foreign_schedule_ID +"</option>");
				});
			}
	...
	$('#foreignOrderQuery').click(function(){
		$.ajax({
			type : "post",
			url  : "<%=request.getContextPath()%>/foreign_order/FO_Servlet.do",
			//data : {action:"getForeignOrders",foDate:$('foreignOrderDate').val(),item_type:$("#foreign_schedule").val()},
			data : {action:"getForeignOrders",db_id:"DB01",fodate:$('#foreignOrderDate').val(),item_type:$("#foreign_schedule").val()},
			datatype: "json",
			success : function(Jdata){
				$("#foreignOrders").empty();
				$.each(Jdata, function(index, element){
	...
	$('#submit').click(function() {  
		...
		$.ajax({
			...
			url  : "<%=request.getContextPath()%>/foreign_order/FO_Servlet.do",
			//data : {action:"getForeignOrders",foDate:$('foreignOrderDate').val(),item_type:$("#foreign_schedule").val()},
			data : {action:"update",foreignOrderDate:foreignOrderDate, foreign_schedule_id:foreign_schedule_id, foreignOrders:foreignOrders,emp_id:"${account.emp_id}"},

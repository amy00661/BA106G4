...
public class OrderDAO implements OrderDAO_interface {
	...
	private static final String UPDATE_STMT = "UPDATE ORDER_MAIN SET EMP_ID=?, MEMBER_ID=?, DB_ID=?, ORDER_STATUS =?, PAYMENT_TYPE=?, FEE=?, EXTRAFEE=?, ITEM_SIZE=?, ITEM_WEIGHT=?,"
			+ "ITEM_TYPE=?, RECEIVER_NAME=?, RECEIVER_TEL=?, RECEIVER_CELL=?, RECEIVER_CITY=?, RECEIVER_COUNTY=?, RECEIVER_ADDRESS=?, "
	...
	private static final String UPDATE_STATUS_DB_STMT = "UPDATE ORDER_MAIN SET ORDER_STATUS= ?, DB_ID= ?, WHERE ORDER_ID = ?";
	...
	private static final String GET_ALL_STMT_BY_MEMID = "SELECT * FROM ORDER_MAIN WHERE MEMBER_ID = ? AND ORDER_STATUS <> '已送達' ORDER BY CREATE_TIME DESC";

	private static final String GET_ALL_STMT_BY_MEMID_HISTORY = "SELECT * FROM ORDER_MAIN WHERE MEMBER_ID = ? ORDER BY CREATE_TIME DESC";

	private static final String GET_ALL_STMT_BY_EMPID = "SELECT * FROM ORDER_MAIN WHERE EMP_ID = ? AND ORDER_STATUS = '待收貨'";

	private static final String INSERT_ONE_WITH_ORDERID_LO = "INSERT INTO LOCAL_ORDER(LOCAL_ORDER_ID, EMP_ID, ORDER_ID, LOCAL_ORDER_UPDATETIME) VALUES ('LO'||LPAD(TO_CHAR(local_order_seq.NEXTVAL), 3, '0'), ?, ?, SYSDATE)";

	private static final String INSERT_ONE_WITH_ORDERID_FO = "INSERT INTO FOREIGN_ORDER(FOREIGN_ORDER_ID, EMP_ID, ORDER_ID, FO_UPDATETIME) VALUES ('FO'||LPAD(TO_CHAR(foreign_order_seq.NEXTVAL), 3, '0'), ?, ?, SYSDATE)";
...
//	}
	...
	public List<OrderVO> findByEmp(String emp_id) {
		...
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement(GET_ALL_STMT_BY_EMPID);
	...
	public Integer UpdateStatusDbByPrimaryKey(String order_status, String db_id, String order_id) {
		...
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement(UPDATE_STATUS_DB_STMT);
	...
	public Integer insertOneWithOrderIDLo (String emp_id, String order_id) {
		...
		Integer showResult = 0;
		
		try {
			
			con = ds.getConnection();
			pstmt = con.prepareStatement(INSERT_ONE_WITH_ORDERID_LO);
			pstmt.setString(1, emp_id);
	...
	public Integer insertOneWithOrderIDFo (String emp_id, String order_id) {
		...
		try {
			...
			con = ds.getConnection();
			pstmt = con.prepareStatement(INSERT_ONE_WITH_ORDERID_FO);
			pstmt.setString(1, emp_id);
			pstmt.setString(2, order_id);
